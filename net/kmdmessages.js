...
function createStructs (overrideTypes) {
  ...
  return createMessages({
    // Data Messages
    block: struct([
      { name: 'header', type: types.kmdheader },
      { name: 'transactions', type: struct.VarArray(varint, types.kmdtransaction) }
    ]),
    getblocks: struct([
    ...
    /*getnSPV: struct([
      { name: 'nspvReq', type: types.nspvReq }
    ]),*/
    getnSPV: types.varBuffer,
    /*nSPV: struct([
      { name: 'nspvResp', type: types.nspvResp }
    ]),*/
    nSPV: types.varBuffer
  })
}

exports.kmdMessages = createStructs(defaultTypes)
